    ...
    with jsonlines.open(path, mode='a') as writer:
        for key, value in result.items():
            data = {
                ...
                'answer': value[0],
                'iteration': value[1],
                "semantic_change": value[4],
                'eval_results': value[2],
            }
            writer.write(data)
...
eval = evaluator("string")
def gcg_origin(model, instences, suffix_length=30, batch_size=16, suffix_num=32, early_stopping=False, layer=22):
    for module in model.target_model.modules():
        if isinstance(module, torch.nn.Embedding):
            embed = module
    ...
    for i in range(100):
        ...
        for inst in instences:
            ...
            #loss = outputs.loss
            loss = outputs.loss*5 - (outputs.hidden_states[layer][0][inst.L2-1] - inst.origin).flatten() @ direction.flatten() / direction.norm()
        ...
        with torch.no_grad():
            ...
            for inst in instences:
                ...
                for batch in batches:
                    ...
                    temp = gcg_loss(logits, torch.cat([inst.labels]*len(batch)))*5 -(outputs.hidden_states[layer][:, inst.L2-1] - inst.origin).view(-1, 4096) @ direction.flatten() / direction.norm()
                    #temp = gcg_loss(logits, torch.cat([inst.labels] * len(batch)))
                    batch_loss = torch.cat([batch_loss, temp])
                total_loss += batch_loss
            best_token_ids = suffix_id_lst[torch.argmin(total_loss)]
            batch_cost += time.time() - start_time
            print(batch_cost/(i+1))
            ...
            for inst in instences:
                ...
                if i:
                    model.generate_by_token_ids(ids, length=200, steam=False)
                    if early_stopping and eval.llm_detector(inst.prompt, model.generate_text, device="cuda:6"):
                        _pass += 1

                    whole_prompt = \
                    model.tokenizer.batch_decode(model.generation["sequences"][:, :inst.L2], skip_special_tokens=True,
                                                 clean_up_tokenization_spaces=False)[0]
                    # 保存结果
                    result = {
                        inst.prompt: [model.generate_text, i, inst.target in model.generate_text, ids.tolist(),
                                      inst.semantic_change.item()],
                    }
                    evaluation_cost += time.time() - start_time
                    print(evaluation_cost/(i+1))

                    #save_to_jsonl(result, path=f'../result/{model.llm_name}/gcg_{layer}_fullcase.jsonl')
            print(f"pass:{_pass}")
